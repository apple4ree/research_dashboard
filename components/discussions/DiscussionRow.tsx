...
import { CommentIcon, RepoIcon } from '@primer/octicons-react';
    ...
    <li className="px-4 py-3 flex items-center gap-3 border-b border-border-muted last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-fg-muted mt-0.5 flex items-center gap-2 flex-wrap">
          ...
          {!hideProject && discussion.projectSlug && (
            <Link
              href={`/projects/${discussion.projectSlug}`}
              className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-xs border border-border-default text-fg-muted hover:bg-canvas-subtle"
            >
              <RepoIcon size={10} /> {discussion.projectSlug}
            </Link>
          )}
